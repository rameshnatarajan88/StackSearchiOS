...
//    cell.backgroundColor=[UIColor colorWithRed:(250/255.f) green:(249/255.f) blue:(249/255.f) alpha:1.0f];
    cell.question = [queTable valueForKey:@"title"];
    cell.answerCount = [queTable valueForKey:@"answer_count"];
    return cell;
}
- (void) tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    [cell setBackgroundColor:[UIColor colorWithRed:(250/255.f) green:(249/255.f) blue:(249/255.f) alpha:1.0f]];
}
...
//    UIView *bgSubView = [[UIView alloc] initWithFrame:CGRectMake(80, 380, 590, 300)];
//    //bgSubView.backgroundColor=[UIColor whiteColor];
//    [bgSubView setBackgroundColor:[UIColor whiteColor]];
//    bgSubView.opaque=YES;
    UIColor *colorOfButtonBacground = [UIColor orangeColor];
    ...
    {
     choiceFullAnswer.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2-200, self.view.center.y, 140, 40);
    }else{
        choiceFullAnswer.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2-100, self.view.center.y, 110, 40);
    ...
    [choiceFullAnswer setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [choiceFullAnswer setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
    {
    choiceAcceptedAnswer.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2+60, self.view.center.y, 140, 40);
    }else{
       choiceAcceptedAnswer.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2+20, self.view.center.y, 140, 40);
    ...
    [choiceAcceptedAnswer setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [choiceAcceptedAnswer setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
    ...
    //[transparetSubView addSubview:bgSubView];
...
-(IBAction)showFullAnswer:(id)sender {
    ...
    if ([connection checkInternetConnection]) {
        ...
        {
            UIWebView *webView = [[UIWebView alloc] initWithFrame:CGRectMake(self.view.bounds.origin.x, self.view.bounds.origin.y+60, self.view.bounds.size.width, self.view.bounds.size.height-60)];
            webView.tag = 2;
            webView.delegate=self;
            [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:_currentQuestionUrl]]];
            [self.view addSubview:webView];
            UIView *choiceViewToRemove1 = [self.view viewWithTag:6];
            [self.view bringSubviewToFront:choiceViewToRemove1];
